...
public class TruffulaOptions {
  ...
  public TruffulaOptions(String[] args) throws IllegalArgumentException, FileNotFoundException {
    // test for null and empty args and checking if their are too many args
    if (args == null || args.length == 0 || args.length > 3) {
      throw new IllegalArgumentException("Invalid arguments");
    }

    // set our varible to default values and then we can change them
    boolean showHiddenValue = false;
    boolean useColorValue = true;

    // we go through all args except the last one to check for any flags and set our
    // varibles
    ...
    // the last arg should be the path to the directory
    ...
    // if we got here then we can set our varibles to the values we got final
    root = directory;
